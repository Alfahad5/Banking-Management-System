...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($amount) || $amount <= 0) {
        $alertMessage = 'Please enter a valid deposit amount.';
    } else {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($row['account_email'] === $email && $row['pin'] === $pin && $row['account_type'] === $account_type) {
                ...
                if ($update_stmt->execute()) {
                    ...
                    $transaction_type = 'Deposit'; // Define the transaction type as 'Deposit'
                    ...
                    if ($insert_transaction_stmt->execute()) {
                        $alertMessage = 'Transaction successful. ' . $amount . ' Deposited';
                    } else {
...
<body>
    ...
    <div class="main-content bg-gray-50  min-h-screen flex items-center justify-center">
        <div class="bg-white bg-opacity-90 p-8 rounded-lg shadow-md w-full max-w-md">
            <h1 class="text-2xl font-bold mb-6 text-center">Deposit Funds</h1>
            <form method="POST">
                <div class="mb-4">
                ...
                <div class="mb-4">
                    <label for="amount" class="block text-gray-700">Deposit Amount</label>
                    <input type="text" id="amount" name="amount" class="w-full p-2 border border-gray-300 rounded mt-1"
                        required>
                </div>

                <button type="submit"
                    class="w-full bg-blue-500 text-white p-2 rounded hover:bg-blue-600">Deposit</button>
            </form>
        </div>
    </div>
